...
public class StringUtil {
    ...
    public static String appendString(String... args) {
        ...
        for (String strArg : args) {
            if (!TextUtils.isEmpty(strArg)) {
                strBuffer.append(strArg);
            }
        }
        return strBuffer.toString();
    }

    //拼接字符串  1 变成 "01. "  10 变成  "10. "   100 变成 "100."
    public static String getLessonOrderString(int lessonOrder) {
        StringBuffer strBuffer = new StringBuffer();
        if (lessonOrder > 0 && lessonOrder < 10) {
            strBuffer.append(String.valueOf(0)).append(String.valueOf(lessonOrder)).append(". ");
        } else if (lessonOrder >= 10 && lessonOrder < 100) {
            strBuffer.append(String.valueOf(lessonOrder)).append(". ");
        } else if (lessonOrder >= 100) {
            strBuffer.append(String.valueOf(lessonOrder)).append(".");
        }
        return strBuffer.toString();
    }
    ...
    public static String toURLEncoded(String paramString) {
        if (paramString == null || paramString.equals("")) {
            Log.e("test", "toURLEncoded error:" + paramString);
            return "";
        }
        ...
        try {
            ...
            Log.e("test", "toURLEncoded error:" + paramString + localException);
        }
    ...
    public static SpannableStringBuilder setTextViewSpanColor(String str, int color, int fstart, int fend) {
        ...
        if (str != null && fstart <= fend && fend <= str.length()) {
